<!--
目的：作为简历二维码访问者的公开项目入口，快速展示劈瓜的产品价值、图谱能力与工程边界。
定义：面向招聘方和面试官的个人项目案例页；详细安装和本地验证信息下沉到折叠区。
范围包括：项目定位、冻结展示样例、个人贡献、可验证工程实现与公开数据边界。
...
> **个人全链路主导的 AI 产品经理项目。** 输入一份 JD，输出业务链路、职责边界、岗位风险与面试核验问题。
...
- `/sample` 使用冻结 fixture 渲染生产结果页，不调用模型、不写浏览器历史，便于验收展示效果。
- `make ci` 先执行公开边界检查，再运行编译与单元测试；真实输入、密钥、日志和本机路径不能进入 Git 跟踪文件。

<details>
<summary><strong>查看本地体验、Skill 安装与验证说明</strong></summary>
...
访问 `http://127.0.0.1:8000/sample` 即可查看不调用 LLM 的冻结样例。真实 JD 分析需要调用者通过环境变量自行配置 API Key。
...
在对应工具中开启新会话后，使用 `$ai-pm-jd-analyzer`。

### 验证口径

- 历史 MVP 的 5-case 回归 run 曾将 `element_modeling` 最佳 score 从 **0.9333** 提升到 **0.9500**，无 timeout 或执行失败。
- 该结果来自旧私有原型的历史回归摘要，未包含 capability 集，不能视为模型准确率、用户效果或商业指标；完整限定见[脱敏验证摘要](HISTORICAL_MVP_VALIDATION.md)。
