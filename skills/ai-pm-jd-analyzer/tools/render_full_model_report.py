...
<body>
  ...
  (() => {
    ...
    const open=(id,trigger) => { const item=all[id]; if(!item) return; lastTrigger=trigger || document.activeElement; drawerContent.replaceChildren(make('p','model-detail-type',types[id] || '审阅结论'),make('h3','',item.name || id)); const description=item.description || item.category || item.domain || item.content_category; if(description) drawerContent.append(make('p','',description)); if(types[id]==='工作事项') { const stream=streams[item.value_stream_id], operations=(item.entity_operations || []).map((operation) => `${labels[operation.operation] || operation.operation} · ${entities[operation.entity_id] && entities[operation.entity_id].name || operation.entity_id}`), needs=(item.capability_ids || []).map((capabilityId) => capabilities[capabilityId] && capabilities[capabilityId].name || capabilityId), raci=(model.responsibility_assignments || []).filter((assignment) => assignment.work_item_id===item.id).map((assignment) => `${assignment.raci && assignment.raci.toUpperCase() || 'R'} · ${roles[assignment.role_id] && roles[assignment.role_id].name || assignment.role_id}`); drawerContent.append(make('p','drawer-copy',stream ? `所属价值流：${stream.name}` : '所属价值流：JD 未披露')); drawerContent.append(make('p','drawer-copy',`CRUD 实体：${operations.join('；') || 'JD 未披露'}`),make('p','drawer-copy',`所需能力：${needs.join('、') || 'JD 未披露'}`),make('p','drawer-copy',`责任分配：${raci.join('、') || 'JD 未披露'}`)); } if(types[id]==='业务实体' && item.primary_capability_id) drawerContent.append(make('p','drawer-copy',`主归属能力：${capabilities[item.primary_capability_id] && capabilities[item.primary_capability_id].name || item.primary_capability_id}`)); if(types[id]==='任职要求') drawerContent.append(make('p','drawer-copy',(item.mapping_target_ids || []).length ? `映射对象：${item.mapping_target_ids.map((target) => all[target] && all[target].name || target).join('、')}` : '岗位准入条件：未映射具体模型对象。')); drawerContent.append(evidence(item)); drawer.classList.add('open'); drawer.setAttribute('aria-hidden','false'); drawer.focus(); };
