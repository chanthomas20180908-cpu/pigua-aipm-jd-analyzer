...
def _require_description(item: dict[str, Any], label: str) -> None:
    description = item.get("description")
    if not isinstance(description, str) or not description.strip():
        raise ReportRenderError(f"{label} 缺少业务 description")
...
def validate_model(model: dict[str, Any]) -> None:
    ...
    risks = _index(_require_list(model, "risks"), "risks")
    uncertainties = _index(_require_list(model, "uncertainties"), "uncertainties")
    ...
    for label, items in (
        ...
        for item in items.values():
            _require_description(item, f"{label} {item['id']}")
    ...
    capability_entities: dict[str, set[str]] = {}
    for capability in capabilities.values():
        primary_entities = capability.get("primary_entity_ids")
        if not isinstance(primary_entities, list) or not primary_entities:
            raise ReportRenderError(f"能力 {capability['id']} 缺少主归属实体列表")
        if len(primary_entities) != len(set(primary_entities)):
            raise ReportRenderError(f"能力 {capability['id']} 的主归属实体重复")
        capability_entities[capability["id"]] = set()
        for entity_id in primary_entities:
            _check_reference(entity_id, entities, "能力 primary_entity_ids")
            entity = entities[entity_id]
            if entity.get("primary_capability_id") != capability["id"]:
                raise ReportRenderError(
                    f"能力 {capability['id']} 与实体 {entity_id} 的主归属不一致"
                )
            capability_entities[capability["id"]].add(entity_id)

    for entity in entities.values():
        primary_capability_id = entity.get("primary_capability_id")
        _check_reference(primary_capability_id, capabilities, "实体 primary_capability_id")
        if entity["id"] not in capability_entities[primary_capability_id]:
            raise ReportRenderError(
                f"实体 {entity['id']} 未被主归属能力 {primary_capability_id} 反向引用"
            )
        attributes = entity.get("attributes")
        if not isinstance(attributes, list):
            raise ReportRenderError(f"实体 {entity['id']} 的 attributes 必须是数组")
        for attribute in attributes:
            if not isinstance(attribute, dict):
                raise ReportRenderError(f"实体 {entity['id']} 的属性不是对象")
            attribute_name = attribute.get("name")
            attribute_description = attribute.get("description")
            if not isinstance(attribute_name, str) or not attribute_name.strip():
                raise ReportRenderError(f"实体 {entity['id']} 的属性缺少名称")
            if not isinstance(attribute_description, str) or not attribute_description.strip():
                raise ReportRenderError(f"实体 {entity['id']} 的属性 {attribute_name!r} 缺少业务 description")
            if not isinstance(attribute.get("evidence"), dict):
                raise ReportRenderError(f"实体 {entity['id']} 的属性 {attribute_name!r} 缺少证据")
    ...
    for capability in capabilities.values():
        ...
        if not isinstance(supported, list) or not supported:
            raise ReportRenderError(f"能力 {capability['id']} 缺少支撑工作事项")
        if len(supported) != len(set(supported)):
            raise ReportRenderError(f"能力 {capability['id']} 的支撑工作事项重复")
...
<head>
  ...
  <style>
    :root { --paper:#f4f0e7;--surface:#fffdf7;--ink:#17342b;--muted:#66786e;--line:#d7ddd3;--grid:rgba(29,83,63,.075);--forest:#195c48;--leaf:#31905d;--amber:#dd9226;--brick:#a8482f;--soft:#e8f0e5;--shadow:0 22px 55px rgba(35,57,45,.12);--body:"Avenir Next","PingFang SC","Hiragino Sans GB",sans-serif;--display:"LXGW WenKai Lite","Songti SC",serif; }
    html[data-theme="dark"] { --paper:#101914;--surface:#17241d;--ink:#eef5ee;--muted:#aac0b2;--line:#385043;--grid:rgba(176,220,188,.08);--forest:#8bd3ad;--leaf:#6cc98d;--amber:#f0b661;--brick:#ee947f;--soft:#223b2d;--shadow:0 24px 65px rgba(0,0,0,.3); }
    * { box-sizing:border-box } body { margin:0; min-width:320px; color:var(--ink); background:radial-gradient(circle at 88% -15%,rgba(102,174,116,.18),transparent 30rem),var(--paper); font:14px/1.55 var(--body); } button,a { font:inherit; } button { color:inherit; } button:focus-visible,a:focus-visible,[tabindex="0"]:focus-visible { outline:3px solid var(--amber); outline-offset:3px; }
    ...
    .flow-view-shell,.capability-view-shell,.requirement-view-shell { padding:22px; overflow-x:auto; background-color:var(--surface); background-image:linear-gradient(var(--grid) 1px,transparent 1px),linear-gradient(90deg,var(--grid) 1px,transparent 1px); background-size:28px 28px; } .flow-view-grid,.capability-layout { min-width:960px; } .flow-view-grid { display:grid; grid-template-columns:210px minmax(0,1fr); gap:18px; align-items:start; } .rail { padding:15px; border:1px solid var(--line); border-radius:18px; background:color-mix(in srgb,var(--surface) 93%,var(--soft)); } .rail h3,.relation-zone h3 { margin:0 0 10px; color:var(--forest); font-size:12px; letter-spacing:.06em; } .rail-note,.empty-state { margin:0; color:var(--muted); font-size:12px; } .role-card,.requirement-card,.entity-card,.work-mini,.relation-card { margin-top:9px; border:1px solid var(--line); border-radius:12px; background:var(--surface); } .role-card { padding:11px; } .role-card strong,.requirement-card strong,.entity-card strong,.work-mini strong { display:block; font-size:13px; line-height:1.35; } .role-link,.model-link { width:100%; border:0; background:none; color:var(--ink); cursor:pointer; font:inherit; text-align:left; } .role-link { padding:7px 0 0; color:var(--forest); font-size:12px; } .raci-badge,.relation-pill,.field-status { display:inline-flex; align-items:center; border-radius:999px; padding:2px 6px; font-size:10px; font-weight:900; } .raci-badge { margin-right:5px; color:var(--forest); background:var(--soft); } .relation-pill.crud { color:#925700; background:color-mix(in srgb,var(--amber) 22%,var(--surface)); } .relation-pill.capability { color:var(--brick); background:color-mix(in srgb,var(--brick) 13%,var(--surface)); }
    .stream-stack { display:grid; grid-template-columns:repeat(auto-fit,minmax(245px,1fr)); gap:15px; } .stream-container,.capability-container { position:relative; border:1px solid color-mix(in srgb,var(--forest) 52%,var(--line)); border-radius:20px; padding:15px; background:color-mix(in srgb,var(--surface) 92%,var(--soft)); } .stream-container:before,.capability-container:before { position:absolute; top:-10px; left:14px; padding:2px 7px; border-radius:999px; background:var(--forest); color:#fff; font-size:10px; font-weight:900; letter-spacing:.04em; } .stream-container:before { content:'价值流'; } .capability-container:before { content:'业务能力'; } .stream-container h3,.capability-container h3 { margin:2px 0 12px; font:700 18px/1.22 var(--display); } .work-card { margin-top:10px; padding:11px; border:1px solid var(--line); border-radius:13px; background:var(--surface); } .work-card h4 { margin:0; font-size:13px; line-height:1.35; } .association-row { display:grid; gap:5px; margin-top:9px; padding-top:8px; border-top:1px dashed var(--line); } .association-label { color:var(--muted); font-size:10px; font-weight:900; letter-spacing:.04em; } .association-links { display:flex; flex-wrap:wrap; gap:5px; } .association-link { border:1px solid var(--line); border-radius:999px; padding:4px 7px; background:var(--surface-strong,var(--surface)); color:var(--ink); cursor:pointer; font-size:11px; line-height:1.25; text-align:left; }
    .capability-stack { display:grid; grid-template-columns:repeat(auto-fit,minmax(250px,1fr)); gap:15px; } .entity-card,.work-mini { padding:10px; } .entity-card { border-left:3px solid var(--amber); } .work-mini { border-left:3px solid var(--leaf); } .model-description,.model-attributes { margin:7px 0 0; color:var(--muted); font-size:12px; line-height:1.5; } .model-attributes { color:var(--forest); } .requirement-card { padding:11px; border-left:3px solid var(--brick); } .requirement-card.is-independent { border-left-color:var(--amber); } .requirement-card .association-links { margin-top:8px; } .relation-zone { margin-top:18px; padding:15px; border-top:1px solid var(--line); background:color-mix(in srgb,var(--surface) 90%,var(--soft)); } .relation-list { display:grid; grid-template-columns:repeat(auto-fit,minmax(230px,1fr)); gap:9px; } .relation-card { display:flex; align-items:center; gap:7px; padding:10px; } .relation-card .model-link { padding:0; font-size:12px; } .relation-arrow { flex:0 0 auto; color:var(--forest); font-weight:900; } .relation-card.is-neutral .relation-arrow { color:var(--muted); }
    .requirement-board { display:grid; min-width:820px; gap:12px; } .requirement-map { display:grid; grid-template-columns:minmax(260px,.85fr) minmax(170px,.45fr) minmax(300px,1fr); gap:12px; align-items:center; padding:14px; border:1px solid var(--line); border-radius:18px; background:color-mix(in srgb,var(--surface) 94%,var(--soft)); } .requirement-map.is-independent { grid-template-columns:minmax(260px,.85fr) minmax(170px,.45fr) minmax(220px,.7fr); } .requirement-trigger { width:100%; border:0; background:none; color:var(--ink); cursor:pointer; text-align:left; padding:0; } .requirement-trigger strong { display:block; font-size:14px; line-height:1.35; } .requirement-meta { display:flex; flex-wrap:wrap; gap:5px; margin-top:7px; } .requirement-meta span { border-radius:999px; padding:2px 6px; background:var(--soft); color:var(--forest); font-size:10px; font-weight:900; } .requirement-bridge { color:var(--muted); font-size:11px; font-weight:900; text-align:center; } .requirement-bridge:before { content:'→'; margin-right:6px; color:var(--brick); font-size:16px; vertical-align:-1px; } .requirement-targets { display:flex; flex-wrap:wrap; gap:6px; } .requirement-targets .association-link { border-color:color-mix(in srgb,var(--brick) 42%,var(--line)); }
    ...
    @media print { .tools,.graph-toolbar,.drawer-scrim,.detail-drawer { display:none!important; } .shell { width:100%; padding:0; } .intro,.model-workbench { box-shadow:none; } #graph-svg { height:580px; } }
...
<body>
  ...
  (() => {
    ...
    const counts = [['价值流',model.value_streams.length],['工作事项',model.work_items.length],['业务实体',model.business_entities.length],['业务能力',model.business_capabilities.length]];
    counts.forEach(([label,value]) => { const card=make('div','metric'); card.append(make('b','',String(value)),make('span','',label)); $('#metrics').append(card); });
    ...
    function linkedNodeButton(id,prefix='') { const linked=nodeById.get(id); if(!linked) return null; const button=make('button','detail-link',`${prefix}${linked.name}`); button.type='button'; button.addEventListener('click',() => { const target=nodeElements.get(id); openDetail(linked,target); target && target.focus(); }); return button; }
    function nodeDescription(node) { const raw=node.raw; return raw.description || ''; }
    ...
    function addSpecificDetails(node,container) { const raw=node.raw; if(node.type==='work') { const stream=linkedNodeButton(raw.value_stream_id,'所属价值流 · '); if(stream) container.append(stream); (raw.capability_ids||[]).forEach((id) => { const button=linkedNodeButton(id,'所需能力 · '); if(button) container.append(button); }); } if(node.type==='entity') { if(raw.primary_capability_id) { const button=linkedNodeButton(raw.primary_capability_id,'主归属能力 · '); if(button) container.append(button); } const attributes=raw.attributes||[]; if(attributes.length) { const attributeSection=section('JD 支持的实体属性'); attributes.forEach((attribute) => attributeSection.append(make('p','drawer-copy',`${attribute.name}：${attribute.description}`))); container.append(attributeSection); } } if(node.type==='capability') (raw.supported_work_item_ids||[]).forEach((id) => { const button=linkedNodeButton(id,'支撑工作事项 · '); if(button) container.append(button); }); if(node.type==='work') { (model.responsibility_assignments||[]).filter((item) => item.work_item_id===node.id).forEach((item) => { const role=roles[item.role_id]; if(role) container.append(make('p','drawer-copy',`${labels[item.raci] || item.raci}：${role.name}`)); }); } }
    function openDetail(node,trigger) { state.lastTrigger=trigger||document.activeElement; drawerContent.replaceChildren(); drawerContent.append(make('p','drawer-type',labels[node.type])); drawerContent.append(make('h3','',node.name)); const description=nodeDescription(node); if(description) drawerContent.append(make('p','drawer-copy',description)); const specifics=section('模型映射'); addSpecificDetails(node,specifics); if(specifics.children.length>1) drawerContent.append(specifics); const relations=section('关联元素'); relations.append(relationshipRows(node)); drawerContent.append(relations); const evidence=section('来源证据'); evidence.append(evidenceBlock(node.raw.evidence)); drawerContent.append(evidence); nodeElements.forEach((element) => element.classList.toggle('selected',element===trigger)); drawer.classList.add('open'); drawer.setAttribute('aria-hidden','false'); scrim.classList.add('open'); drawer.focus(); }
    function closeDetail() { drawer.classList.remove('open'); drawer.setAttribute('aria-hidden','true'); scrim.classList.remove('open'); nodeElements.forEach((element) => element.classList.remove('selected')); if(state.lastTrigger&&state.lastTrigger.focus) state.lastTrigger.focus(); }
    ...
    $('#fit-graph').addEventListener('click',() => { state.scale=1;state.tx=0;state.ty=0;applyTransform();updateZoomControls(); }); $('#graph-zoom-in').addEventListener('click',() => setScale(state.scale * ZOOM_FACTOR)); $('#graph-zoom-out').addEventListener('click',() => setScale(state.scale / ZOOM_FACTOR)); $('#drawer-close').addEventListener('click',closeDetail); scrim.addEventListener('click',closeDetail); document.addEventListener('keydown',(event) => { if(event.key==='Escape'&&drawer.classList.contains('open')) closeDetail(); }); $('#theme-toggle').addEventListener('click',() => { const root=document.documentElement,next=root.dataset.theme==='dark'?'light':'dark'; root.dataset.theme=next; $('#theme-toggle').textContent=next==='dark'?'切到白天':'切到夜里'; }); $('#print-button').addEventListener('click',() => window.print());
  ...
  (() => {
    ...
    const showView=(name) => { Object.entries(panels).forEach(([key,panel]) => { panel.hidden=key !== name; }); tabs.forEach((tab) => tab.setAttribute('aria-selected',String(tab.dataset.viewTarget===name))); };
    ...
    const close=() => { drawer.classList.remove('open'); drawer.setAttribute('aria-hidden','true'); if(lastTrigger && lastTrigger.focus) lastTrigger.focus(); };
    ...
    const open=(id,trigger) => { const item=all[id]; if(!item) return; lastTrigger=trigger || document.activeElement; drawerContent.replaceChildren(make('p','model-detail-type',types[id] || '审阅结论'),make('h3','',item.name || id)); if(item.description) drawerContent.append(make('p','',item.description)); if(types[id]==='工作事项') { const stream=streams[item.value_stream_id], operations=(item.entity_operations || []).map((operation) => `${labels[operation.operation] || operation.operation} · ${entities[operation.entity_id] && entities[operation.entity_id].name || operation.entity_id}`), needs=(item.capability_ids || []).map((capabilityId) => capabilities[capabilityId] && capabilities[capabilityId].name || capabilityId), raci=(model.responsibility_assignments || []).filter((assignment) => assignment.work_item_id===item.id).map((assignment) => `${assignment.raci && assignment.raci.toUpperCase() || 'R'} · ${roles[assignment.role_id] && roles[assignment.role_id].name || assignment.role_id}`); drawerContent.append(make('p','drawer-copy',stream ? `所属价值流：${stream.name}` : '所属价值流：JD 未披露')); drawerContent.append(make('p','drawer-copy',`CRUD 实体：${operations.join('；') || 'JD 未披露'}`),make('p','drawer-copy',`所需能力：${needs.join('、') || 'JD 未披露'}`),make('p','drawer-copy',`责任分配：${raci.join('、') || 'JD 未披露'}`)); } if(types[id]==='业务实体') { if(item.primary_capability_id) drawerContent.append(make('p','drawer-copy',`主归属能力：${capabilities[item.primary_capability_id] && capabilities[item.primary_capability_id].name || item.primary_capability_id}`)); (item.attributes || []).forEach((attribute) => drawerContent.append(make('p','drawer-copy',`${attribute.name}：${attribute.description}`))); } if(types[id]==='任职要求') drawerContent.append(make('p','drawer-copy',(item.mapping_target_ids || []).length ? `映射对象：${item.mapping_target_ids.map((target) => all[target] && all[target].name || target).join('、')}` : '岗位准入条件：未映射具体模型对象。')); drawerContent.append(evidence(item)); drawer.classList.add('open'); drawer.setAttribute('aria-hidden','false'); drawer.focus(); };
    $('#model-detail-close').addEventListener('click',close); document.addEventListener('keydown',(event) => { if(event.key==='Escape') close(); });
    ...
    const link=(id,text,kind='',stayInView=false) => { const button=make('button',`association-link ${kind}`,text || all[id] && all[id].name || id); button.type='button'; button.addEventListener('click',() => { const target=targetView(id); if(target && !stayInView) showView(target); requestAnimationFrame(() => open(id,button)); }); return button; };
    ...
    const renderValue=() => {
      const rail=$('#roles-rail'), board=$('#value-stream-board'), assignments=model.responsibility_assignments || []; rail.replaceChildren(make('h3','','责任角色')); board.replaceChildren();
      if(!(model.roles || []).length) rail.append(empty('JD 未建模出角色责任分配。'));
      (model.roles || []).forEach((role) => { const card=make('article','role-card'); card.append(make('strong','',role.name)); const own=assignments.filter((assignment) => assignment.role_id===role.id); if(!own.length) card.append(empty('JD 未明确其负责事项。')); own.forEach((assignment) => { const work=works[assignment.work_item_id]; if(work) card.append(association(`${assignment.raci && assignment.raci.toUpperCase() || 'R'} · 责任事项`,[link(work.id,work.name)])); }); rail.append(card); });
      if(!(model.value_streams || []).length) board.append(empty('JD 未建模出价值流。'));
      (model.value_streams || []).forEach((stream) => { const container=make('section','stream-container'); container.append(make('h3','',stream.name)); const streamWorks=(model.work_items || []).filter((work) => work.value_stream_id===stream.id); if(!streamWorks.length) container.append(empty('该价值流没有已建模工作事项。')); streamWorks.forEach((work) => { const card=make('article','work-card'); card.append(make('h4','',work.name)); const operations=(work.entity_operations || []).map((operation) => link(operation.entity_id,`${labels[operation.operation] || operation.operation} · ${entities[operation.entity_id] && entities[operation.entity_id].name || operation.entity_id}`,'crud',true)); if(operations.length) card.append(association('CRUD 实体',operations)); const needs=(work.capability_ids || []).map((id) => link(id,capabilities[id] && capabilities[id].name || id,'capability',true)); if(needs.length) card.append(association('所需能力',needs)); const workRoles=assignments.filter((assignment) => assignment.work_item_id===work.id).map((assignment) => link(assignment.role_id,`${assignment.raci && assignment.raci.toUpperCase() || 'R'} · ${roles[assignment.role_id] && roles[assignment.role_id].name || assignment.role_id}`)); card.append(workRoles.length ? association('责任分配',workRoles) : empty('JD 未明确责任角色。')); container.append(card); }); board.append(container); });
    };
    ...
    const renderCapability=() => {
      ...
      (model.business_capabilities || []).forEach((capability) => { const container=make('section','capability-container'); container.append(make('h3','',capability.name)); container.append(make('p','model-description',capability.description)); const owned=(model.business_entities || []).filter((entity) => entity.primary_capability_id===capability.id); if(!owned.length) container.append(empty('没有主归属到该能力的业务实体。')); owned.forEach((entity) => { const card=make('article','entity-card'); card.append(make('strong','',entity.name),make('p','model-description',entity.description)); if(entity.domain) card.append(make('p','empty-state',entity.domain)); const attributes=(entity.attributes || []).map((attribute) => attribute.name); if(attributes.length) card.append(make('p','model-attributes',`JD 支持属性：${attributes.join('、')}`)); card.addEventListener('click',(event) => open(entity.id,event.currentTarget)); container.append(card); }); const supported=(capability.supported_work_item_ids || []).map((id) => works[id]).filter(Boolean); if(supported.length) container.append(association('支撑工作事项',supported.map((work) => link(work.id,work.name,'',true)))); board.append(container); });
