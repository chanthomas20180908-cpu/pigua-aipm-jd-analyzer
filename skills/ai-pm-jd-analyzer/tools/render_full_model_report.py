    ...
    for entity in entities.values():
        ...
        for attribute in attributes:
            ...
            if not isinstance(attribute_description, str) or not attribute_description.strip():
                raise ReportRenderError(
                    f"实体 {entity['id']} 的属性 {attribute_name!r} 缺少业务 description"
                )
...
<body>
  ...
  (() => {
    ...
    const renderV2EntityDetails=() => { (model.business_capabilities || []).forEach((capability,index) => { const container=$('#capability-board').children[index]; if(!container) return; if(capability.description) container.insertBefore(make('p','model-description',capability.description),container.children[1] || null); const owned=(model.business_entities || []).filter((entity) => entity.primary_capability_id===capability.id); [...container.querySelectorAll('.entity-card')].forEach((card,entityIndex) => { const entity=owned[entityIndex]; if(!entity) return; if(entity.description) card.append(make('p','model-description',entity.description)); const attributes=(entity.attributes || []).map((attribute) => attribute.name); if(attributes.length) card.append(make('p','model-attributes',`JD 支持属性：${attributes.join('、')}`)); }); }); };
    renderValue(); renderCapability(); renderV2EntityDetails(); renderRequirements(); renderDetails(); showView('relationship');
  })();
  </script>
</body>
</html>'''
